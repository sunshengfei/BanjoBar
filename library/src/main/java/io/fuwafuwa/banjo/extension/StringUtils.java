...
public final class StringUtils {
    ...
    public static float extractNumber(String itemStr, float defValue) {
        if (RegexHelper.isEmpty(itemStr)) return defValue;
        Pattern pattern = Pattern.compile("(\\d+(\\.\\d+)?)");
        Matcher matcher = pattern.matcher(itemStr);
        float speed = defValue;
        if (matcher.find()) {
            speed = Float.parseFloat(matcher.group());
        }
        return speed;
    }
